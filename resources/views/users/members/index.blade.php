...
@endphp
<x-layouts.main-content title="Members" heading="List of members" subheading="Manage members">
    <div class="flex gap-4 w-full flex-1 flex-col rounded-xl">
        <div>
            <form method="GET" action="{{ route('users.members') }}">
                @csrf
                <div class="flex justify-between">
                    <div class="w-full flex flex-wrap gap-4">
                        <div>
                            <flux:input icon="magnifying-glass" clearable name="search" :value="$search ?? ''" placeholder="Search members">
                    ...
                    <div class="grow-0 flex flex-row space-y-3 justify-start gap-4">
                        <flux:dropdown align="center">
                            ...
                            <flux:navmenu>
                                <flux:menu.radio.group>
                                    @foreach (['created_at', 'name', 'card', 'email', 'nif', 'gender'] as $orderBy)
                        ...
                        <flux:button.group>
                            ...
                            <flux:button variant="outline" :href="route('users.members')" class="w-10"><flux:icon.x-mark/></flux:button>
                        </flux:button.group>
                    </div>
                </div>
            </form>
                    ...
                    <table class="w-full table-auto border-collapse">
                        ...
                        <thead>
                            <tr class="bg-zinc-100 dark:bg-zinc-700 text-gray-800 dark:text-gray-200 font-semibold rounded-lg shadow-xs ring-1 ring-zinc-200 dark:ring-zinc-600">
                                ...
                                <th class="px-2 py-1 text-left">NIF</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-b-zinc-100 dark:border-b-zinc-700">
                                ...
                                <td class="px-2 py-2 text-left">
                                    @if ($member->card)
                                        @if ($member->card->deleted_at)
                                            <flux:badge size="sm" icon="credit-card">
                                                {{ $member->card->card_number }}
                                            </flux:badge>
                                        @else
                                            <flux:badge size="sm" :color="$badgeColors[$member->card->card_number % $badgeColors->count()]" icon="credit-card">
                                                {{ $member->card->card_number }}
                                            </flux:badge>
                                        @endif
                                    @else
                                ...
                                <td class="px-2 py-2 text-left">
                                    <span class="inline-flex items-center gap-2">
                                        <flux:link href="mailto:{{ $member->email }}">{{ $member->email }}</flux:link>
                                ...
                                <td class="px-2 py-2 text-left">
                                    ...
                                    @elseif ($member->type == 'pending_member')
                                        <flux:badge variant="pill" size="sm" color="blue">Pending</flux:badge>
                                    @else
                                ...
                                <td class="text-left">
                                    @if ($member->blocked)
                                        <flux:modal :name="'unblock-member-'.$member->id" class="min-w-[22rem]">
                                            <div class="space-y-6">
                                                <div>
                                                    <flux:heading size="lg" class="flex gap-2 mr-8">
                                                        <flux:icon name="lock-open"/>
                                                        Unblock member "{{ $member->name }}"?
                                                    </flux:heading>
                                                    <flux:text class="mt-2">
                                                        <p>You're about to unblock this member.</p>
                                                    </flux:text>
                                                </div>
                                                <div class="flex gap-2">
                                                    ...
                                                    <form method="POST" action="{{ route('users.unblock', ['user' => $member->id]) }}" class="flex items-center">
                                                        @csrf
                                                        @method('PUT')
                                                        <flux:button type="submit" variant="danger">Unlock</flux:button>
                                                    </form>
                                                </div>
                                            </div>
                                        </flux:modal>
                                    @else
                                        <flux:modal :name="'promote-member-employee-'.$member->id" class="min-w-[22rem]">
                                            <div class="space-y-6">
                                                <div>
                                                    <flux:heading size="lg" class="flex gap-2 mr-8">
                                                        <flux:icon name="shield-check"/>    
                                                        Promote "{{ $member->name }}" to Employee?
                                                    </flux:heading>
                                                    <flux:text class="mt-2">
                                                        <p>You're about to promote this member to the rank of employee.</p>
                                                    </flux:text>
                                                </div>
                                                <div class="flex gap-2">
                                                    <flux:spacer />
                                                    ...
                                                    <form method="POST" id="form-promote-member-employee-{{ $member->id }}" class="flex items-center">
                                                        @csrf
                                                        @method('DELETE')
                                        ...
                                        <flux:modal :name="'promote-member-board-'.$member->id" class="min-w-[22rem]">
                                            <div class="space-y-6">
                                                <div>
                                                    <flux:heading size="lg" class="flex gap-2 mr-8">
                                                        <flux:icon name="briefcase"/>    
                                                        Promote "{{ $member->name }}" to Board?
                                                    </flux:heading>
                                                    <flux:text class="mt-2">
                                                        <p>You're about to promote this member to the rank of board.</p>
                                                    </flux:text>
                                                </div>
                                                <div class="flex gap-2">
                                                    <flux:spacer />
                                                    ...
                                                    <form method="POST" action="{{ route('users.promote', ['user' => $member->id]) }}" class="flex items-center">
                                                        @csrf
                                                        @method('PUT')
                                                        <flux:button type="submit" variant="primary">Promote</flux:button>
                                                    </form>
                                        ...
                                        <flux:modal :name="'block-member-'.$member->id" class="min-w-[22rem]">
                                            <div class="space-y-6">
                                                <div>
                                                    <flux:heading size="lg" class="flex gap-2 mr-8">
                                                        <flux:icon name="no-symbol"/>
                                                        Block member "{{ $member->name }}"?
                                                    </flux:heading>
                                                    <flux:text class="mt-2">
                                                        <p>You're about to block this member.</p>
                                                    </flux:text>
                                                </div>
                                                <div class="flex gap-2">
                                                    <flux:spacer />
                                                    ...
                                                    <form method="POST" action="{{ route('users.block', ['user' => $member->id]) }}" class="flex items-center">
                                                        @csrf
                                                        @method('PUT')
                                            ...
                                            <div class="space-y-6">
                                                <div>
                                                    <flux:heading size="lg" class="flex gap-2 mr-8">
                                                        ...
                                                        Delete "{{ $member->name }}"'s account?</flux:heading>
                                                    <flux:text class="mt-2">
                                                        <p>You're about to delete this member's account.</p>
                                                ...
                                                <div class="flex gap-2">
                                                    ...
                                                    <form method="POST" action="{{ route('users.delete', ['user' => $member->id]) }}" class="flex items-center">
                                                        @csrf
                                                        @method('DELETE')
                                    ...
                                    <flux:dropdown class="w-48" align="end">
                                        ...
                                        <flux:menu>
                                            <flux:menu.item icon="eye" :href="route('users.view', ['user' => $member])">View</flux:menu.item>
                                            @if ($member->id != auth()->user()->id)
                                                <flux:menu.item icon="pencil-square" class="cursor-pointer" :href="route('users.edit', ['user' => $member])">Edit</flux:menu.item>
                                                @if ($member->blocked)
                                                    <flux:modal.trigger :name="'unblock-member-'.$member->id">
                                                        <flux:menu.item icon="lock-open" class="cursor-pointer">Unblock</flux:menu.item>
                                                    </flux:modal.trigger>
                                                @else
                                                    <flux:modal.trigger :name="'promote-member-board-'.$member->id">
                                                        <flux:menu.item icon="briefcase" class="cursor-pointer">Promote to Board</flux:menu.item>
                                                    </flux:modal.trigger>
                                                    <flux:modal.trigger :name="'block-member-'.$member->id">
                                                        <flux:menu.item icon="no-symbol" variant="danger" class="cursor-pointer">Block</flux:menu.item>
                                                    </flux:modal.trigger>
                                                @endif
                            ...
                            @endforeach
                             <tr class="border-b-zinc-400 dark:border-b-zinc-500 text-gray-600 dark:text-gray-400">
                                <td class="px-2 py-2 text-left rounded-b-lg" colspan="8">
                                    <x-pagination :paginator="$members" paginatorName="members"/>
                                </td>
                            </tr>
                        </tbody>
                    </table>
