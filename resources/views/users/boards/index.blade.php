...
@endphp
<x-layouts.main-content title="Boards" heading="List of boards" subheading="Manage boards">
    <div class="flex gap-4 w-full flex-1 flex-col rounded-xl">
        <div>
            <form method="GET" action="{{ route('users.boards') }}">
                @csrf
                <div class="flex justify-between">
                    <div class="w-full flex flex-wrap gap-4">
                        <div>
                            <flux:input icon="magnifying-glass" clearable name="search" :value="$search ?? ''" placeholder="Search boards">
                    ...
                    <div class="grow-0 flex flex-row space-y-3 justify-start gap-4">
                        <flux:dropdown align="center">
                            ...
                            <flux:navmenu>
                                <flux:menu.radio.group>
                                    @foreach (['created_at', 'name', 'card', 'nif', 'email', 'gender', 'best'] as $orderBy)
                        ...
                        <flux:button.group>
                            ...
                            <flux:button variant="outline" :href="route('users.boards')" class="w-10"><flux:icon.x-mark/></flux:button>
                        </flux:button.group>
                    </div>
                </div>
            </form>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-b-zinc-100 dark:border-b-zinc-700">
                                <td class="px-2 py-2 text-left">
                                    <flux:avatar size="xs" src="{{ $board->photo_url }}" />
                                </td>
                                <td class="px-2 py-2 text-left">
                                    {{ strtoupper($board->name) }}
                                </td>
                                <td class="px-2 py-2 text-left">
                                    @if ($board->card)
                                        @if ($board->card->deleted_at)
                                            <flux:badge size="sm" icon="credit-card">
                                                {{ $board->card->card_number }}
                                            </flux:badge>
                                        @else
                                            <flux:badge size="sm" :color="$badgeColors[$board->card->card_number % $badgeColors->count()]" icon="credit-card">
                                                {{ $board->card->card_number }}
                                            </flux:badge>
                                        @endif
                                    @else
                                ...
                                <td class="px-2 py-2 text-left">
                                    <span class="inline-flex items-center gap-2">
                                        <flux:link href="mailto:{{ $board->email }}">{{ $board->email }}</flux:link>
                                        @if ($board->email_verified_at)
                                ...
                                <td class="px-2 py-2 text-left">
                                    ...
                                    @elseif ($board->type == 'pending_member')
                                ...
                                <td class="text-left">
                                    @if ($board->blocked)
                                        <flux:modal :name="'unblock-board-'.$board->id" class="min-w-[22rem]">
                                            <div class="space-y-6">
                                                <div>
                                                    <flux:heading size="lg" class="flex gap-2 mr-8">
                                                        <flux:icon name="lock-open"/>
                                                        Unblock board "{{ $board->name }}"?
                                                    </flux:heading>
                                                    <flux:text class="mt-2">
                                                        <p>You're about to unblock this board.</p>
                                                    </flux:text>
                                                </div>
                                                <div class="flex gap-2">
                                                    ...
                                                    <form method="POST" id="form-unblock-board-{{ $board->id }}" class="flex items-center">
                                                        @csrf
                                                        @method('DELETE')
                                    ...
                                    @else
                                        <flux:modal :name="'demote-board-member-'.$board->id" class="min-w-[22rem]">
                                            <div class="space-y-6">
                                                <div>
                                                    <flux:heading size="lg" class="flex gap-2 mr-8">
                                                        <flux:icon name="user"/>
                                                        Demote "{{ $board->name }}" to Member?
                                                    </flux:heading>
                                                    <flux:text class="mt-2">
                                                        <p>You're about to demote this board to the rank of member.</p>
                                                    </flux:text>
                                                </div>
                                                <div class="flex gap-2">
                                                    <flux:spacer />
                                                    ...
                                                    <form method="POST" action="{{ route('users.demote', ['user' => $board->id]) }}" class="flex items-center">
                                                        @csrf
                                                        @method('PUT')
                                        ...
                                        <flux:modal :name="'demote-board-employee-'.$board->id" class="min-w-[22rem]">
                                            <div class="space-y-6">
                                                <div>
                                                    <flux:heading size="lg" class="flex gap-2 mr-8">
                                                        <flux:icon name="shield-check"/>
                                                        Demote "{{ $board->name }}" to Employee?
                                                    </flux:heading>
                                                    <flux:text class="mt-2">
                                                        <p>You're about to demote this board to the rank of employee.</p>
                                                    </flux:text>
                                                </div>
                                                <div class="flex gap-2">
                                                    <flux:spacer />
                                                    ...
                                                    <form method="POST" id="form-demote-board-employee-{{ $board->id }}" class="flex items-center">
                                                        @csrf
                                                        @method('DELETE')
                                                        <flux:button type="submit" variant="primary">Demote</flux:button>
                                                    </form>
                                        ...
                                        <flux:modal :name="'block-board-'.$board->id" class="min-w-[22rem]">
                                            <div class="space-y-6">
                                                <div>
                                                    <flux:heading size="lg" class="flex gap-2 mr-8">
                                                        <flux:icon name="no-symbol"/>
                                                        Block board "{{ $board->name }}"?
                                                    </flux:heading>
                                                    <flux:text class="mt-2">
                                                        <p>You're about to block this board.</p>
                                                    </flux:text>
                                                </div>
                                                <div class="flex gap-2">
                                                    <flux:spacer />
                                                    ...
                                                    <form method="POST" id="form-block-board-{{ $board->id }}" class="flex items-center">
                                                        @csrf
                                                        @method('DELETE')
                                    ...
                                    @endif
                                    <flux:modal :name="'delete-board-'.$board->id" class="min-w-[22rem]">
                                            <div class="space-y-6">
                                                <div>
                                                    <flux:heading size="lg" class="flex gap-2 mr-8">
                                                        <flux:icon name="trash"/>
                                                        Delete "{{ $board->name }}"'s account?</flux:heading>
                                                    <flux:text class="mt-2">
                                                        <p>You're about to delete this board's account.</p>
                                                ...
                                                <div class="flex gap-2">
                                                    ...
                                                    <form method="POST" action="{{ route('users.delete', ['user' => $board->id]) }}" class="flex items-center">
                                                        @csrf
                                                        @method('DELETE')
                                    ...
                                    <flux:dropdown class="w-48" align="end">
                                        ...
                                        <flux:menu>
                                            <flux:menu.item icon="eye" :href="route('users.view', ['user' => $board])">View</flux:menu.item>
                                            @if ($board->id != auth()->user()->id)
                                                <flux:menu.item icon="pencil-square" class="cursor-pointer" :href="route('users.edit', ['user' => $board])">Edit</flux:menu.item>
                                                <flux:modal.trigger :name="'demote-board-member-'.$board->id">
                                                    <flux:menu.item icon="user" class="cursor-pointer">Demote to Member</flux:menu.item>
                                                </flux:modal.trigger>
                                                <flux:modal.trigger :name="'delete-board-'.$board->id">
                            ...
                            <tr class="border-b-zinc-400 dark:border-b-zinc-500 text-gray-600 dark:text-gray-400">
                                <td class="px-2 py-2 text-left rounded-b-lg" colspan="8">
                                    <x-pagination :paginator="$boards" paginatorName="boards"/>
                                </td>
                            </tr>
                        </tbody>
                    </table>
